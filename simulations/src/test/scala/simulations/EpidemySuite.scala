...
class EpidemySuite extends FunSuite {
  ...
  test("neighbours"){
    ...
    person.row=0
    person.col=0
    assert(neighbours(person.position).toSet == Set((7,0),(0,7),(0,1),(1,0)), "neighbours 1")
    person.row=7
    person.col=7
    assert(neighbours(person.position).toSet == Set((6,7),(7,6),(7,0),(0,7)), "neighbours 2")
  }
  
  test("Infected position"){
        val es = new EpidemySimulator
    import es._
    import es.SimConfig._
    
    assert((for {
      i <- 0 to roomRows - 1
      j <- 0 to roomColumns -1
    } yield (i,j).isInfected).forall(_ == false),"empty positions should be clear of infection")
  }
